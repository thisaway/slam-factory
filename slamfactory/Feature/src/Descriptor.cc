...
static int staticRandomPointsPair31[256 * 4] = {
...
public:
    ...
    BriefDescriptorImpl(int numComparedPoints, int descriptionLength,  \
    ...
    inline void setAdjacentAreaRadius(int adjacentAreaRadius) SF_OVERRIDE {
        ...
        aar = adjacentAreaRadius;
        makeRandomPointsPair();
    }

    inline int getNumComparedPoints() const SF_OVERRIDE {return ncp;};
    inline void setNumComparedPoints(int numComparedPoints) SF_OVERRIDE {
        
        assert(numComparedPoints == 2);
        ncp = numComparedPoints;
        makeRandomPointsPair();
    }
...
makeRandomPointsPair(){
    ...
    if(rt == BriefDescriptor<_Length>::UNIFORM){
        ...
        for(int i = 0, pairLength = dl * ncp *2; i < pairLength; ++i){

                pointsPair[i] = uniformDistrib(rng);
        }
    }
    else{
...
#define BRIEF_COMPAIR(index)  \
    img(static_cast<int>(px + pointsPair[index] * cosa), static_cast<int>(py + pointsPair[index + 1] * sina))  \
...
computeDescription(const Image& img,  \
    ...
    size_t il;
    int desL = dl / BRIEF_BIT_EACH_INT;
    int pi;
    double angle, sina, cosa;
    int px, py;

    descriptions.resize(ol);
    for(size_t i = 0; i < ol; ++i){
        ...
        for(size_t j = 0; j < il; j++){
            ...
            px = keypoints[i][j].x;
            py = keypoints[i][j].y;
            if(px < aar or py < aar)continue;

            angle = keypoints[i][j].orientation;
            sina = sin(angle);
            cosa = cos(angle);
            pi = 0;

            for(int d = 0; d < desL; ++d){
